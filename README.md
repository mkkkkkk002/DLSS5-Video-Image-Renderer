...
**v1.5 更新亮点**
- **渲染参数内嵌成品**：所有输出（视频 mp4/mkv、图片 png/jpg）都会把本次渲染参数
  写入文件（容器 comment / PNG tEXt / JPG COM，不影响播放器与看图器）
- **拖放自动恢复**：参数卡下方新增拖放框——把本软件输出的成品文件拖入（或点击
  选择）即自动恢复当初的画面参数；配「撤销 / 重做」（按钮 + Ctrl+Z / Ctrl+Y）防误操作
- **图片渲染次数**：图片卡独立「渲染次数」（仅管「渲染此图」与图片批量），
  「快速渲染当前帧」跟随视频板块的渲染次数
- **NV-OF 质量档位可选**：SLOW（默认，最准）/ MEDIUM / FAST
